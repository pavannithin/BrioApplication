...
function ItemMbl(props) {
  return (
    <Box sx={{ height: '37vh', width: '100%' }}>

      <Grid sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', rowGap: '2rem' }}>
        <Grid item sx={{maxHeight: '30vh'}}>
          <Typography variant='h6'>"{props.item.description}"</Typography>
        </Grid>
...
function showTransmission(isDeskTopDevice, deskTopitems, mobileitems) {
  if (!isDeskTopDevice) {
    return mobileitems.map((item, i) => <ItemMbl key={i} item={item} />)
  } else {
...
function ReviewsContainer(props) {
  ...
  return (
    <Grid sx={{
      ...
      <Grid item sx={{ padding: '0rem 3rem 2rem 3rem' }}>
